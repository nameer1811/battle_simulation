...
- ~~Add a scripted sweep under `scripts/` that writes scenario summaries for a wider grid of `mexican-concentration` and `cos-fatigue` values.~~ Done: `scripts/run-grid-sweep.sh` and `scripts/analyze_results.py`.
